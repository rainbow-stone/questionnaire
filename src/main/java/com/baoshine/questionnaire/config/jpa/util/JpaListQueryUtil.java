...
public abstract class JpaListQueryUtil extends BaseJpaUtil {
	 ...
	 *
	 * @param <T>
	 *            the object type of the item in the returned list
	 * @param queryHints
	 *            the query hints
	 * @param entityManager
	 *            the entity manager
	 ...
	 *            the query hints
	 * @param baseJpql
	 *            it also can be a template JPQL that contains {@link BaseJpaUtil#WHERE_CLAUSE_TOKEN} and
	...
	public static <T> List<T> listQuery(JpaQueryHints queryHints, EntityManager entityManager, Class<T> itemType,
			String baseJpql, JpaFragment... jpaFragments) throws EntityPersistenceException {
		try {
			final List<Parameter> parameters = new ArrayList<>();
	...
	public static <T> List<T> listLimitGeneralQuery(JpaQueryHints queryHints, EntityManager entityManager,
			Class<T> itemType, String jpql, int firstResult, int maxResults, List<?> parameters)
			throws EntityPersistenceException {
		try {
	...
	public static <T> List<T> listLimitQuery(JpaQueryHints queryHints, EntityManager entityManager,
			Class<T> entityClass, int firstResult, int maxResults, JpaFragment... jpaFragments)
			throws EntityPersistenceException {
		try {
			final List<Parameter> parameters = new ArrayList<>();
	...
	public static <T> List<T> listLimitQuery(JpaQueryHints queryHints, EntityManager entityManager, Class<T> itemType,
			String baseJpql, int firstResult, int maxResults, JpaFragment... jpaFragments)
			throws EntityPersistenceException {
		try {
			final List<Parameter> parameters = new ArrayList<>();
