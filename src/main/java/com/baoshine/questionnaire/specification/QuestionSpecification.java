...
public class QuestionSpecification {
    ...
    public static Specification<Question> buildCondition(String code, Long questionType, Long presentationType,
                                                         String content) {
        return (root, query, builder) -> {
            List<Predicate> predicates = new ArrayList<>();
            Optional.ofNullable(code).ifPresent(item -> predicates.add(builder.equal(root.get("code"), item)));
            Optional.ofNullable(questionType)
                    .ifPresent(item -> predicates.add(builder.equal(root.get("questionType"), item)));
            Optional.ofNullable(presentationType)
                    .ifPresent(item -> predicates.add(builder.equal(root.get("presentationType"), item)));
